 ...
 * Type for list items.
 */
export interface TabListItem extends HTMLElement {
  disabled?: boolean;
  selected?: boolean;
  variant?: string;
}
...
export class TabList extends LitElement {
  ...
  protected static navigationKeys = new Map([
  ...
  @state() private items: TabListItem[] = [];
  ...
  constructor() {
    ...
    this.addEventListener('keydown', async (event: KeyboardEvent) => {
      const {key} = event;
      const shouldHandleKey = TabList.navigationKeys.get('default')!.has(key) ||
          TabList.navigationKeys.get(this.orientation)!.has(key);
  ...
  private findFocusableItem(i = -1, prev = false, tries = 0): TabListItem|null {
  ...
  protected updateFocusableItem(item: HTMLElement|null) {
  ...
  protected async handleItemClick(event: Event) {
    const {target} = event;
    if (await this.wasEventPrevented(event)) {
      return;
    }
    const item =
        (target as Element).closest(`${this.localName} > *`) as TabListItem;
